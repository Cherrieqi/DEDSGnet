...
from networks.DEDSGnet import DEDSGnet
from torch.utils.data import DataLoader
from loss import simi_cal, SDPloss
from ImageDataset_H1318 import ImgDataset_train
...
img_1 = np.load("data/H1318/gen_H18/img.npy")
img_1 = torch.from_numpy(img_1).float()
label_1 = np.load("data/H1318/gen_H18/gt.npy")
label_1 = torch.LongTensor(label_1)

img_2 = np.load("data/H1318/gen_H13/img.npy")
img_2 = torch.from_numpy(img_2).float()
label_2 = np.load("data/H1318/gen_H13/gt.npy")
...
slice_size = 3
in_ch = 103
out_ch_cede = [256, 128, 128]
out_ch_ifem = [128, 128, 256, 256]
out_ch = [256, 64]
class_num = 4


work_dir = f'./work_dir/H1318_inch{in_ch}_w{w[0]}-{w[1]}-{w[2]}-{w[3]}_b{batch_size}_lr{learning_rate}/' \
...
model = DEDSGnet(in_ch=in_ch, out_ch=out_ch, out_ch_cede=out_ch_cede, out_ch_ifem=out_ch_ifem, class_num=class_num, slice_size=slice_size).to(device)
...
for epoch in range(0, num_epoch):
    ...
    for i, data in loop:
        ...
        x_img_1, x_ex_img_1, y_1, side_1_1, side_1_2, side_1_3, side_1_4,\
        y_ex_1, side_ex_1_1, side_ex_1_2, side_ex_1_3, side_ex_1_4 = \
            model(data[0].to(device), mode='trian')

        x_img_2, x_ex_img_2, y_2, side_2_1, side_2_2, side_2_3, side_2_4, \
        y_ex_2, side_ex_2_1, side_ex_2_2, side_ex_2_3, side_ex_2_4 = \
            model(data[2].to(device), mode='trian')
        ...
        # loss 4
        loss4 = loss_cons(x_img_1, x_ex_img_1) + \
                loss_cons(x_img_2, x_ex_img_2)
        ...
        loop.set_postfix(cls_ls=loss1.item(), dif_ls=loss2.item(), prog_ls=loss3.item(), cons_ls=loss4.item(),
