...
from ImageDataset_PUPC import ImgDataset_test
...
work_dir = f"./work_dir/PUPC_inch103_w3-1-1-2_b1024_lr0.015/{seed}/"
for j in range(2):
    print(f"seed: {seed}")
    if j == 0:
        data_name = "H13"
    ...
    if data_name == "H13":
        img = np.load("data/PUPC/gen_H13/img.npy")
        label = np.load("data/PUPC/gen_H13/gt.npy")
    else:
        img = np.load("data/PUPC/gen_H18/img.npy")
        label = np.load("data/PUPC/gen_H18/gt.npy")
    ...
    model = DEDSGnet(in_ch=in_ch, out_ch=out_ch, out_ch_cede=out_ch_cede, out_ch_ifem=out_ch_ifem, class_num=class_num,
                 slice_size=slice_size).to(device)
    ...
    if data_name == "H13":
        label_vision_1d(pred_total, row_col_total, 349, 1905, result_save_path + "pred_PUPC_H13.png")
    else:
        label_vision_1d(pred_total, row_col_total, 2384, 601, result_save_path + "pred_PUPC_H18.png")
