...
def set_shuffle(image_slice, label_slice):
    """
    :param image_slice:  [N, c, slice_size, slice_size]
    :param label_slice:
    :return: image_slice_shuffle, label_slice_shuffle :
    """
    ...
    image_slice_shuffle = torch.full([N, c, slice_size, slice_size], 0.)
    label_slice_shuffle = torch.full([N], 0)
    for i in range(N):
        image_slice_shuffle[i] = image_slice[idx[i]]
        label_slice_shuffle[i] = label_slice[idx[i]]

    return image_slice_shuffle, label_slice_shuffle, idx
...
def set_division(class_num, class_list, image_slice, label_slice, mode, rate, row_col):
    ...
    # train/val/test
    image_ex = torch.full([N, c, slice_size, slice_size], 0.).float()
